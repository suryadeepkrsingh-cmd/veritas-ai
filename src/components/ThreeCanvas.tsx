...
const GlobeNodes = () => {
  ...
  const { positions, sizes, colors } = useMemo(() => {
    const nodeCoords = [
      [40.7, -74.0], [37.7, -122.4], [51.5, -0.1], [48.8, 2.3], [28.6, 77.2], [19.0, 72.8],
      [35.6, 139.7], [1.3, 103.8], [-33.9, 151.2], [52.5, 13.4], [25.2, 55.3], [55.7, 37.6],
    ...
    nodeCoords.forEach(([lat, lon], index) => {
      const point = globePoint(lat, lon, 2.28);
      pos[index * 3] = point.x;
      pos[index * 3 + 1] = point.y;
      pos[index * 3 + 2] = point.z;

      size[index] = 10 + (index % 4) * 2;

      colorObj.set(palette[index % palette.length]);
      color[index * 3] = colorObj.r;
      color[index * 3 + 1] = colorObj.g;
      color[index * 3 + 2] = colorObj.b;
    });

    return { positions: pos, sizes: size, colors: color };
  }, []);

  useFrame((state) => {
    if (pointsRef.current) {
      pointsRef.current.rotation.y = state.clock.elapsedTime * 0.14;
      ...
      <shaderMaterial
        ...
        vertexShader={`
          ...
          void main() {
            ...
            float pulse = 0.85 + 0.25 * sin(uTime * 2.0 + position.x * 2.0 + position.y * 1.5);
            gl_Position = projectionMatrix * mvPosition;
            gl_PointSize = aSize * pulse * (120.0 / -mvPosition.z);
          }
        `}
        fragmentShader={`
          varying vec3 vColor;
